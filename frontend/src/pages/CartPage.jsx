...
export default function CartPage() {
    ...
    // PHASE 1 CHANGE: New state for the two checkout fields the cashier selects.
    // deliveryType defaults to "IN_STORE" so the existing flow is unchanged.
    ...
    async function handleCheckout() {
        ...
        try {
            // PHASE 1 CHANGE: Pass the delivery/payment options into the checkout call.
            // For IN_STORE orders, deliveryAddress will be an empty string which the
            // backend treats the same as undefined (no address stored).
            await orderApi.checkout(key, {
                deliveryType,
    ...
    // Payment method options change depending on delivery type.
    // Cash on delivery is only available for remote orders.
    const paymentOptions = deliveryType === "IN_STORE"
        ? ["CASH", "UPI", "CARD"]
        : ["CASH_ON_DELIVERY"]
            ...
            {loading ? (
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div className="lg:col-span-2 card p-5">
                        ...
                        {items.map(item => (
                            <CartItemRow key={item.id} item={item} onRemove={handleRemove} removing={removing} />
                        ))}
                    </div>
                    ...
                    <div className="flex flex-col gap-4">
                        <div className="card p-5">
                            ...
                            {/* PHASE 1 CHANGE: Delivery type selector.
                                This is a simple toggle — IN_STORE or DELIVERY.
                                Selecting DELIVERY reveals the address input and
                                locks payment method to Cash on Delivery. */}
                            <div className="mb-4">
                                <p className="text-xs text-muted mb-2">Order type</p>
                                <div className="flex rounded-lg overflow-hidden border border-border">
                                    {["IN_STORE", "DELIVERY"].map(type => (
                                        <button
                                            key={type}
                                            onClick={() => {
                                                setDeliveryType(type)
                                                setPaymentMethod(type === "IN_STORE" ? "CASH" : "CASH_ON_DELIVERY")
                                            }}
                                        ...
                                        >
                                            {type === "IN_STORE" ? "In-store" : "Delivery"}
                                        </button>
                                    ))}
                                </div>
                            </div>

                            {/* PHASE 1 CHANGE: Address input — only shown for DELIVERY orders */}
                            {deliveryType === "DELIVERY" && (
                                <div className="mb-4">
                                    <label className="text-xs text-muted mb-1.5 block">Delivery address</label>
                                    <textarea
                            ...
                            {/* PHASE 1 CHANGE: Payment method selector.
                                For IN_STORE: Cash, UPI, Card.
                                For DELIVERY: locked to Cash on Delivery (Phase 1). */}
                            <div className="mb-5">
                                <p className="text-xs text-muted mb-2">Payment method</p>
